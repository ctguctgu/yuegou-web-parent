        ...
        <el-dialog title="配置显示属性" v-model="viewPropertiesVisible" :close-on-click-modal="false">
            <el-card class="box-card">
                <div v-for="viewProperty in viewProperties" :key="viewProperty" class="text item">
                    <el-row :gutter="20">
                        <el-col :span="4">{{viewProperty.specName}}:</el-col>
                        <el-col :span="20">
                            <el-input auto-complete="off" v-model="viewProperty.value" style="width: 200px"></el-input>
                        </el-col>
                    </el-row>
                </div>
            </el-card>
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click.native="SaveViewProperties" :loading="editLoading">提交</el-button>
            </div>
        </el-dialog>
        <!--sku属性对话框界面-->
        <el-dialog title="配置SKU属性" v-model="skuPropertiesVisible" :close-on-click-modal="false">
			<el-card class="box-card">
				<div v-for="skuProperty in skuProperties" :key="skuProperty" class="text item">
					...
					<div v-for="i in skuProperty.options.length+1" :key="i" class="text item">
						<el-input auto-complete="off" v-model="skuProperty.options[i-1]" style="width: 400px"></el-input>
						<el-button type="danger" @click="skuProperty.options.splice(i-1,1)" icon="el-icon-delete" plain>删除</el-button>
					</div>
				</div>
			</el-card>
			...
			<div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click.native="SaveSkuProperties" :loading="editLoading">提交</el-button>
            </div>
        </el-dialog>
    </section>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            SaveViewProperties() {
                ...
                this.$confirm('确认提交吗？', '提示', {}).then(() => {
                    this.$http.post("/product/product/saveViewProperties", param).then(res => {
                        this.addLoading = false;
                        let {success, message} = res.data;
                        if (success) {
                            this.$message({
        ...
        watch: {
            skuProperties: {
				handler: function (curV, oldV) {
					let arr = this.skuProperties.filter(s => s.options.length > 0);
					let result = arr.reduce((pre,cur,curIndex)=>{
						let tem = [];
						pre.forEach(e1=>{
